...
# Defect found and fixed 2026-08-28. Silent Mode (step 5) connects as
# ${DB_USER} to build the schema -- never as postgres. `createdb -O` makes
# ${DB_USER} the database owner, but does not reliably make it the *schema*
# owner: `public` is cloned from template1, and on some PostgreSQL versions
# stays owned by the bootstrap superuser. When that happens Silent Mode's own
# migration fails with "permission denied for schema public" and, in
# production runtime-mode, there is no maintenance-mode fallback to fall back
# to -- this is exactly what forced the first real install here to import a
# schema dump by hand as the postgres superuser, which left all 103 tables
# and 2 functions owned by postgres instead of ${DB_USER} ("permission denied
# for table ..." on every subsequent query). Fix the precondition instead of
# working around the symptom: make ${DB_USER} own the schema before
# FusionAuth ever connects to it.
schema_owner=$(runuser -u postgres -- psql -d "${DB_NAME}" -tAc \
...
if [ "$schema_owner" = "${DB_USER}" ]; then
    echo "public schema already owned by ${DB_USER}"
else
    table_count=$(runuser -u postgres -- psql -d "${DB_NAME}" -tAc \
        "select count(*) from pg_tables where schemaname = 'public'")
    if [ "$table_count" != "0" ]; then
        fail "database ${DB_NAME} already has ${table_count} table(s) in schema
       'public', owned by '${schema_owner}' instead of '${DB_USER}'. This is
       the known-bad state from a manual postgres-owned schema import.
       Refusing to reassign ownership of a non-empty schema automatically --
       verify by hand which objects should belong to ${DB_USER} before running:
         ALTER SCHEMA public OWNER TO ${DB_USER};
         REASSIGN OWNED BY ${schema_owner} TO ${DB_USER};   -- if postgres owns rows too"
    fi
    runuser -u postgres -- psql -q -v ON_ERROR_STOP=1 -d "${DB_NAME}" \
        -c "alter schema public owner to ${DB_USER};" >/dev/null
    echo "public schema was owned by '${schema_owner}' on an empty schema -- reassigned to ${DB_USER}"
fi
...
if grep -q '^database.password=' "$FA_CONFIG" 2>/dev/null \
   && ! grep -q '^database.password=fusionauth$' "$FA_CONFIG"; then
    ...
    if ! grep -q '^fusionauth-app\.silent-mode=true$' "$FA_CONFIG"; then
        cat <<'NOTE'
NOTE: this file predates the 2026-08-28 silent-mode fix and does not set
      fusionauth-app.silent-mode=true. If its schema was already migrated
      (by Silent Mode or by hand), nothing is broken today -- but re-reading
      this file into a NEW empty database would hit the same "production mode
      has no migration path" defect this fix closes. Add the line by hand if
      this file is ever regenerated from scratch.
NOTE
    fi
else
    [ -n "${DB_PASSWORD:-}" ] || fail \
    ...
    #
    # silent-mode=true is not optional here. FusionAuth's own docs: "When in
    # production runtime mode, maintenance mode will never run" -- and without
    # Silent Mode there is then no path left that builds the schema at all, so
    # the app never starts against an empty database. Silent Mode's default
    # only becomes true when database.root.username is *also* set, which is
    # deliberately omitted below (root credentials exist only to let
    # FusionAuth create ${DB_USER} for a user that cannot do so itself --
    # ${DB_USER} already owns its database and schema, see step 2, so a
    # superuser fallback would be unused attack surface). Set it explicitly
