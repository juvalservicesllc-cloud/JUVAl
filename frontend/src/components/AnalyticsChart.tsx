...
interface AnalyticsDatum {
  label: string
  value: number
  // Optional per-bar color (CSS color or var(--token)) -- e.g. decision
...
export function AnalyticsChart({ data, chartType }: { data: AnalyticsDatum[]; chartType: ChartType }) {
  ...
  const chart = chartType === "line"
    ...
    : <BarChart data={data} margin={{ top: 12, right: 12, left: 0, bottom: 0 }}>{shared}<Bar dataKey="value" fill="var(--accent)" radius={[4, 4, 0, 0]} animationDuration={240}>{data.some((d) => d.color) && data.map((d) => <Cell key={d.label} fill={d.color ?? "var(--accent)"} />)}</Bar></BarChart>
