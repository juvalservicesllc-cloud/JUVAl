...
| H-14 | Production service hardening | ADR-018 / ADR-031 | `systemctl cat fusionauth-app`; `systemctl show fusionauth-app -p User,NoNewPrivileges,ProtectSystem,...` | **Became applicable 2026-08-27** — `fusionauth-app.service` is now `active`+`enabled`. Verified read-only: unit is `Type=simple`, `User=fusionauth`/`Group=fusionauth` (a `-r` `nologin` account per the package `postinst`), **not root**; `Restart=always`. The systemd sandboxing directives (`ProtectSystem`, `NoNewPrivileges`, `PrivateTmp`, etc.) are **not** set by the vendor unit — a hardening drop-in is not yet added | Vendor unit has no sandboxing directives; `fusionauth.properties` (`runtime-mode`, heap, `search.type`) is root-only and `NOT_VERIFIED` by the agent. A `systemd` hardening drop-in and an `install.sh` reconciliation run are the open items | Agent (service/user facts), User (`sudo` for the drop-in and `install.sh`) | **PARTIAL** (2026-08-27 — reopened from `NOT_APPLICABLE`; runs non-root, but not hardened or reconciled to the runbook) |
...
|---|---|---|---|
| H-17 | Identity data backup with a tested restore | Users, hashed credentials and JWT signing keys are the first data on this host **not** regenerable from Git. `deploy/fusionauth/backup.sh` dumps and verifies with `pg_restore --list`; restore is a documented manual procedure | `NOT_IMPLEMENTED` — **the instance is now live (2026-08-27), so there IS data to lose** (the first admin, the tenant signing key). `backup.sh` reviewed, logic sound; system timer units git-tracked at `tools/systemd/juval-fusionauth-backup.{service,timer}`. **Not scheduled, never run** — `/var/backups/juval-fusionauth` does not exist; needs user `sudo`. `host_monitor.sh` now warns while this is true |
...
| H-19 | Capacity alerting covers the new services | H-11's thresholds were sized for a host running no persistent service. A JVM plus PostgreSQL on 2 cores changes the baseline; `tools/host_monitor.sh` must be re-checked against it | `PARTIAL` (2026-08-27) — `tools/host_monitor.sh` gained a conditional `fusionauth` check (service + PostgreSQL `active` + `/api/status` Ok → PASS/FAIL) and a `fusionauth.backup` freshness check (WARN if no dump < 48h). Runs no-op on a clean node. **Still open**: steady-state CPU-contention headroom under a real JVM + PostgreSQL on 2 cores is `NOT_VERIFIED` — H-11's load thresholds (WARN ≥1.0×, FAIL ≥2.0×) were not re-tuned for the new baseline, and no measurement under concurrent `pytest`/`npm build` exists yet |
...
### 11.6 Measured runtime state, 2026-08-27 (agent-executed, read-only)

Phase 1 is deployed. The clean-slate table in §11.4 is superseded:

| Fact | Value | Relevance |
|---|---|---|
| `fusionauth-app` | `active` + `enabled` | H-14 reopened to `PARTIAL` |
| PostgreSQL | `active`; database `fusionauth`, 103 tables | schema imported manually (§33.2 of the remediation doc), not via `install.sh` |
| Listeners | `:22` (all-if), `:9011` (all-if), **`:9012` (all-if)**, `:5432` (loopback) | `:9012` also serves the full FusionAuth app — **not in ADR-031/README**; covered by the same UFW default-deny (no `allow` rule), confirmed unreachable from `192.168.0.3` per ESTADO CONFIRMADO. Documentation gap, now recorded in ADR-027 §"Fronteras de red" and ADR-031 |
| `/api/status` | `{"status":"Ok"}` on both ports | up |
| OIDC discovery / JWKS | well-formed, RS256, 1 key with `kid` | `tools/verify_oidc.py --issuer http://127.0.0.1:9011` exit 0 — **local `http://` issuer, configuration evidence only** |
| `/usr/local/fusionauth/` | `fusionauth:root`, `0700`/`0600` — unreadable | `fusionauth.properties`, JDK path and `runtime-mode` are `NOT_VERIFIED`; no `sudo` |
| Java | `/usr/local/fusionauth/java/current` → `jdk-25.0.3+9` (from operator shell history, not agent-verified) | matches the version `start.sh` for 1.69.0 pins; provenance (checksum) unverified because the manual install path is unknown |
| Capacity | 13 GiB RAM (85% available with the JVM running), load 0.03× on 4 cores at rest | fits at rest; steady-state contention under `pytest`/`npm build` is H-19's open item |
| Backup | `/var/backups/juval-fusionauth` absent | H-17 not scheduled |
...
`docs/DEVELOPMENT_ENVIRONMENT.md` §4, `tools/host_monitor.sh` (H-15/H-19),
`tools/systemd/` (git-tracked units: `juval-host-monitor` user timer/service
added 2026-08-26; `juval-fusionauth-backup` **system** timer/service for H-17
