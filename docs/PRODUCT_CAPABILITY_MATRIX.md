...
|---|---|---|---|---|
| Amazon matching | ASIN lookup from UPC/EAN/GTIN | Amazon SP-API Catalog Items (DOC VERIFIED; developer registration REJECTED_REMEDIATION_REQUIRED; live call blocked) | Remediation, a truthful reapplication in a new case, and approval are required before a production client, roles and self-authorization; provider+operation rate policy | 0 exact → `NOT_FOUND`; 1 exact → `VERIFIED` candidate; >1 → `AMBIGUOUS`, never auto-picked or downgraded to `NOT_FOUND` |
...
|---|---|---|---|
| ASIN matching | Supplier identifier plus Amazon catalog candidate | SP-API Catalog documentation verified; private-developer registration is REJECTED_REMEDIATION_REQUIRED, so production client, self-authorization and live validation are blocked pending remediation, new case and approval | Model the `AMBIGUOUS` outcome separately from `VerificationStatus`. |
