...
// Reject private namespaces before Vite's SPA fallback, including missing files.
vite.middlewares.use((req,res,next)=>{
 let path;
 try{path=decodeURIComponent((req.url??'/').split('?')[0]).replaceAll('\\','/');}
 catch{res.statusCode=400;res.end('Invalid path');return;}
 if(path.split('/').some(part=>['data','.git','.verification'].includes(part)||part.startsWith('.env'))||path.endsWith('.pem')){
  res.statusCode=403;res.end('Private path');return;
 }
 next();
});
vite.middlewares.use('/api/state' ,(req,res)=>{res.setHeader('Content-Type','application/json');res.setHeader('Cache-Control','no-store');try{res.end(readFileSync(output));}catch{res.statusCode=503;res.end(JSON.stringify({error:'No snapshot available. Sync the repository to begin.'}));}});vite.middlewares.use('/api/sync',async(req,res)=>{
